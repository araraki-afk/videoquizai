    ...
    <div className="dashboard page-container">
      <header className="dashboard-header" style={{ marginBottom: '2rem' }}>
        <div>
          ...
          <p className="subtitle" style={{ color: 'rgba(255,255,255,0.9)' }}>Ваша статистика и доступные материалы</p>
        </div>
      ...
      <section className="pending-tests" style={{ marginBottom: '3rem' }}>
        <div className="section-header">
          <h2 style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>📋 Ваши тесты</h2>
        </div>
      ...
      <section className="completed-tests">
        <div className="section-header">
          <h2 style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>✅ Пройденные тесты</h2>
        </div>
    ...
    <div className="test-card content-card" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
        ...
        <span style={{
          ...
          {isCompleted ? 'Завершён' : 'Новый'}
        </span>
      </div>

      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', color: '#64748b', fontSize: '0.9rem', marginBottom: '1.5rem', flexGrow: 1 }}>
      ...
      {isCompleted && (
        <div style={{ marginBottom: '1.5rem' }}>
          ...
          <div className="score-bar" style={{ width: '100%', height: '6px', background: '#f1f5f9', borderRadius: '3px', overflow: 'hidden' }}>
            <div style={{ width: `${test.score}%`, height: '100%', background: isSuccess ? '#10b981' : '#ef4444' }}></div>
          </div>
        </div>
      )}
      ...
      <div style={{ marginTop: 'auto' }}>
        {!isCompleted ? (
          <Link to={`/test/${test.id}`} className="btn-generate" style={{ width: '100%', display: 'block', textAlign: 'center', padding: '0.8rem', textDecoration: 'none', margin: 0 }}>
            Пройти тест 🚀
          </Link>
        ) : (
          <Link to={`/test/${test.id}`} className="btn-nav btn-back" style={{ width: '100%', display: 'block', textAlign: 'center', padding: '0.8rem', textDecoration: 'none', boxSizing: 'border-box' }}>
            Пройти ещё раз
          </Link>
        )}
      </div>
